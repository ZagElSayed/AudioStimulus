...
# Visualization: Figure 2 - PAC Boxplot
import seaborn as sns
sns.set(style="whitegrid")

# Simulated PAC data
conditions = ['7 Hz', '9 Hz', '11 Hz', '13 Hz']
means = [0.12, 0.18, 0.25, 0.34]
stds = [0.04, 0.035, 0.03, 0.025]
...
for condition, mean, std in zip(conditions, means, stds):
    ...
    for value in pac_values:
        pac_data.append({'Frequency': condition, 'PAC (Alpha-Gamma)': value})

pac_df = pd.DataFrame(pac_data)

plt.figure(figsize=(8, 5))
sns.boxplot(x='Frequency', y='PAC (Alpha-Gamma)', data=pac_df, palette='Set2')
sns.stripplot(x='Frequency', y='PAC (Alpha-Gamma)', data=pac_df, color='black', size=3, alpha=0.6)
